...
public class EntityResolvedTuple<T> extends AbstractEnumerable<T> {
	...
	public EntityResolvedTuple(HashMap<Integer, Object[]> data, UnionFind uFind, 
		...
		// Populate finalData with candidate pairs from similarities
		for (Map.Entry<Integer, HashMap<Integer, Double>> entry : similarities.entrySet()) {
			Integer id1 = entry.getKey();
			for (Integer id2 : entry.getValue().keySet()) {
				Object[] pair = new Object[2];
				pair[0] = id1;
				pair[1] = id2;
				this.finalData.add((T) pair);
			}
		}
	}
	...
	public Enumerator<T> enumerator() {
		// If finalData is empty but we have similarities, return candidate pairs instead
		if(finalData.isEmpty() && similarities != null && !similarities.isEmpty()) {
			return enumerateCandidatePairs();
		}
	...
	/**
	 * Returns an enumerator for candidate pairs (id1, id2) from similarities HashMap
	 * Used when DEDUP query should return pairs instead of merged entities
	 */
	@SuppressWarnings("unchecked")
	private Enumerator<T> enumerateCandidatePairs() {
		List<Object[]> pairs = new ArrayList<>();
		
		// Convert similarities HashMap to list of [id1, id2] pairs
		for (Map.Entry<Integer, HashMap<Integer, Double>> entry : similarities.entrySet()) {
			Integer id1 = entry.getKey();
			for (Integer id2 : entry.getValue().keySet()) {
				// Create array with just id1, id2
				Object[] pair = new Object[2];
				pair[0] = id1;
				pair[1] = id2;
				pairs.add(pair);
		...
		Enumerator<Object[]> pairEnumerator = Linq4j.enumerator(pairs);
		return (Enumerator<T>) pairEnumerator;
	}
