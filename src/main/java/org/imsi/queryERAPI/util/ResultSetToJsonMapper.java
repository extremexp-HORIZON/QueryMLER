...
//            jArray.add(jsonObject);
    ...
    public static List<ObjectNode> mapCRS(ResultSet rs, int start, int end) throws SQLException
    {
        ...
        // Reset cursor to beginning before reading
        try {
            rs.beforeFirst(); // Move cursor before first row
        } catch (SQLException e) {
            // If beforeFirst fails, try absolute positioning
        }
        
        // Check if start position is valid and resultset has data
        try {
            if (start < 1 || !rs.absolute(start)) {
                return jList; // Return empty list if invalid position
            }
        } catch (SQLException e) {
            return jList; // Return empty list on error
        }
